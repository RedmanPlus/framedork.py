def insert_html_into_html(parent_file: str, child_file: str) -> str:
...
def extend_html(filename: str) -> str:
    ...
    if html[0].startswith('{%') and html[0].endswith('%}'):
        ...
        result = insert_html_into_html(contents[2].replace("'", ""), filename)
    else:
        result = ''.join(html)
...
def insert_values_into_html(filename: str, values: dict) -> str:
    a = extend_html(filename)
